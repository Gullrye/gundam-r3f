...
export function ControlPanel({ pose, onPoseChange, autoRotate, onAutoRotateChange, colors, onColorChange }: ControlPanelProps) {
  ...
  return (
    <div style={{
      ...
      background: 'rgba(0,0,0,0.75)', color: '#fff', padding: 12,
      borderRadius: 12, fontFamily: 'sans-serif', minWidth: isOpen ? 200 : 'auto',
      ...
      >
        <span style={{ fontWeight: 'bold', fontSize: 14 }}>Gundam Controls</span>
        <span style={{ fontSize: 10, opacity: 0.6, marginLeft: 12 }}>{isOpen ? '▲' : '▼'}</span>
      </div>
